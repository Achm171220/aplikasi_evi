...
class BerkasAktif extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('berkas_aktif/index', $data);
    }
    ...
    public function listData()
    {
        if ($this->request->isAJAX()) {
            ...
            foreach ($result['data'] as $item) {
                ...
                // Tombol Aksi
                $btn_detail = '<a href="' . site_url('berkas-aktif/detail/' . $item['id']) . '" class="btn btn-sm btn-info" data-bs-toggle="tooltip" title="Lihat Detail Isi Berkas"><i class="bi bi-eye-fill"></i></a>';
                $btn_edit = '';
                $btn_delete = '';
                $btn_pemberkasan = '';
                $btn_toggle_status = '';

                if (has_permission('cud_arsip')) { // Asumsi hak akses ini ada
                    $btn_edit = '<a href="' . site_url('berkas-aktif/edit/' . $item['id']) . '" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" title="Edit Berkas"><i class="bi bi-pencil-fill"></i></a>';

                    $form_delete = '<form action="' . site_url('berkas-aktif/delete/' . $item['id']) . '" method="post" class="d-inline form-delete">
                    ...
                    if ($item['status_tutup'] === 'terbuka') {
                        $btn_pemberkasan = '<a href="' . site_url('berkas-aktif/pemberkasan/' . $item['id']) . '" class="btn btn-sm btn-success" data-bs-toggle="tooltip" title="Tambah Item ke Berkas"><i class="bi bi-plus-circle-fill"></i></a>';

                        $form_tutup = '<form action="' . site_url('berkas-aktif/tutup/' . $item['id']) . '" method="post" class="d-inline form-toggle-status">
                        ...
                        $form_buka = '<form action="' . site_url('berkas-aktif/buka/' . $item['id']) . '" method="post" class="d-inline form-toggle-status">
    ...
    public function tutupBerkas($id = null)
    {
        ...
        $berkas = $this->berkasAktifModel->find($id);
        if (!$berkas) {
            session()->setFlashdata('error', 'Berkas tidak ditemukan.');
            return redirect()->to('/berkas-aktif');
        }

        // 2. Hitung jumlah item di dalam berkas
        $jumlahItem = $this->itemAktifModel->where('id_berkas', $id)->countAllResults();
        ...
        try {
            $this->berkasAktifModel->update($id, ['status_tutup' => 'tertutup']);
    ...
    {
        $this->berkasAktifModel->update($id, ['status_tutup' => 'terbuka']);
    ...
    public function new()
    {
        if (!has_permission('cud_arsip')) {
            return redirect()->to('/berkas-aktif')->with('error', 'Anda tidak memiliki izin untuk menghapus data.');
        }
        $data = [
        ...
        return view('berkas_aktif/form', $data);
    }
    ...
    public function create()
    {
        ...
        $formRules = [
            ...
            'nama_berkas'     => 'required|min_length[3]',
    ...
    public function update($id = null)
    {
        ...
        if (!$this->validate($this->berkasAktifModel->getValidationRules())) {
            return redirect()->back()->withInput()->with('validation', $this->validator);
        }
        $this->berkasAktifModel->update($id, $this->request->getPost());
    ...
    public function delete($id = null)
    {
        ...
        $itemCount = $this->itemAktifModel->where('id_berkas', $id)->countAllResults();
        if ($itemCount > 0) {
            $this->session->setFlashdata('error', 'Gagal menghapus! Masih ada ' . $itemCount . ' item di dalam berkas ini.');
            return redirect()->to('/berkas-aktif');
        }
        $this->berkasAktifModel->delete($id);
    ...
    {
        $berkas = $this->berkasAktifModel->builder()
            ->select('berkas_aktif.*, klasifikasi.kode as kode_klasifikasi, klasifikasi.nama_klasifikasi, klasifikasi.umur_aktif, klasifikasi.umur_inaktif, klasifikasi.nasib_akhir')
            ->join('klasifikasi', 'klasifikasi.id = berkas_aktif.id_klasifikasi', 'left')
            ->where('berkas_aktif.id', $id)
            ->get()->getRowArray();
        ...
        $berkas['jumlah_item'] = $this->itemAktifModel->where('id_berkas', $id)->countAllResults();
        ...
        return view('berkas_aktif/detail', $data);
    }
    ...
    {
        if ($this->request->isAJAX()) {
            $result = $this->itemAktifModel->getDataTablesItemsInBerkas($this->request, $berkasId);
            $data = [];
            foreach ($result['data'] as $item) {
                $row = [];
                $row[] = $item['no_dokumen'];
    ...
    public function pemberkasanPage($berkasId = null)
    {
        ...
        if ($berkas['status_tutup'] === 'tertutup') {
            return redirect()->to('berkas-aktif/detail/' . $berkasId)->with('error', 'Berkas ini sudah ditutup dan tidak bisa ditambahkan item baru.');
        }

        // --- TAMBAHAN BARU: Ambil jumlah item yang sudah ada di berkas ini ---
        $jumlahItemSaatIni = $this->itemAktifModel->where('id_berkas', $berkasId)->countAllResults();
        ...
        return view('berkas_aktif/pemberkasan', $data);
    }
    ...
    public function ajaxListUnfiledItems()
    {
        if ($this->request->isAJAX()) {
            ...
            foreach ($result['data'] as $item) {
                $row = [];
                // --- PERBAIKAN: Kirim ID item sebagai KOLOM PERTAMA (indeks 0) ---
                $row[] = $item['id']; // ID item ini akan digunakan oleh fungsi render untuk checkbox
                $row[] = $item['no_dokumen'] ?? '-';
                $row[] = esc($item['judul_dokumen']);
    ...
    public function addItems($berkasId = null)
    {
        ...
        if (empty($itemIds)) {
            ...
            return redirect()->to('berkas-aktif/pemberkasan/' . $berkasId);
        }
        ...
        try {
            ...
            $cleanItemModel = new \App\Models\ItemAktifModel();
            ...
            if ($db->transStatus() === false) {
                ...
                return redirect()->to('berkas-aktif/detail/' . $berkasId);
            }
        } catch (\Exception $e) {
            $db->transRollback();
    ...
    public function pinjamBerkas()
    {
        ...
        $berkas = $this->berkasAktifModel->find($id_berkas);
        if (!$berkas) {
        ...
        try {
            ...
            // Update status di berkas_aktif
            $this->berkasAktifModel->update($id_berkas, ['pinjam' => 1]);
    ...
    public function prosesImport()
    {
        ...
        if (!$this->validate($validationRules)) {
            ...
            return redirect()->to('/berkas-aktif')->withInput()->with('validation', $this->validator);
        }
        ...
        try {
            ...
            if (!empty($dataToInsert)) {
                $this->berkasAktifModel->insertBatch($dataToInsert);
            }
            ...
            $db->transRollback();
            log_message('error', 'Error prosesImport BerkasAktif: ' . $e->getMessage());
            session()->setFlashdata('show_import_modal', true);
