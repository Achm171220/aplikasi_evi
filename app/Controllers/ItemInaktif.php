...
class ItemInaktif extends BaseController
{
    ...
    public function index()
    {
        ...
        $data = [
            'title'            => 'Manajemen Item Arsip Inaktif',
        ...
        return view('item_inaktif/index', $data);
    }
    ...
    public function listData()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!empty($es3_id_filter)) {
                $this->itemInaktifModel->where('item_inaktif.id_es3', $es3_id_filter); // Filter berdasarkan ID
            } elseif (!empty($es2_id_filter)) {
                $this->itemInaktifModel->where('item_inaktif.id_es2', $es2_id_filter); // Filter berdasarkan ID
            }

            // Panggil getDataTablesList yang akan menerapkan filter hak akses Model
            $result = $this->itemInaktifModel->getDataTablesList($this->request);
            ...
            foreach ($result['data'] as $item) {
                ...
                if (has_permission('cud_arsip_inaktif')) {
                    ...
                    $btn_edit = '<a href="' . site_url('item-inaktif/edit/' . $item['id']) . '" class="btn btn-sm btn-warning" title="Edit"><i class="fas fa-edit"></i> Edit</a>';
                    $btn_lepas = '';
                    if ($item['id_berkas']) {
                    ...
                    $form_delete = '';
                    if (!$item['id_berkas']) {
                        $form_delete = '<form action="' . site_url('item-inaktif/delete/' . $item['id']) . '" method="post" class="d-inline form-delete">'
                    ...
                    $aksi = '<a href="' . site_url('item-inaktif/detail/' . $item['id']) . '" class="btn btn-sm btn-light">Lihat Detail</a>';
                }
    ...
    public function detail($id = null)
    {
        ...
        $item = $this->itemInaktifModel->builder()
            ->select('item_inaktif.*, klasifikasi.kode as kode_klasifikasi, klasifikasi.nama_klasifikasi, jenis_naskah.nama_naskah, es1.nama_es1, es2.nama_es2, es3.nama_es3, users.name as user_creator, berkas_inaktif.nama_berkas')
            ->join('klasifikasi', 'klasifikasi.id = item_inaktif.id_klasifikasi', 'left')
            ->join('jenis_naskah', 'jenis_naskah.id = item_inaktif.id_jenis_naskah', 'left')
            ->join('unit_kerja_es3 as es3', 'es3.id = item_inaktif.id_es3', 'left')
            ->join('unit_kerja_es2 as es2', 'es2.id = item_inaktif.id_es2', 'left')
            ->join('unit_kerja_es1 as es1', 'es1.id = es2.id_es1', 'left')
            ->join('users', 'users.id = item_inaktif.id_user', 'left')
            ->join('berkas_inaktif', 'berkas_inaktif.id = item_inaktif.id_berkas', 'left')
            ->where('item_inaktif.id', $id)
            ->get()->getRowArray();
        ...
        return view('item_inaktif/detail', $data);
    }
    ...
    {
        if (!has_permission('cud_arsip_inaktif')) {
            return redirect()->to('/item-inaktif')->with('error', 'Anda tidak memiliki izin untuk menambah data.');
        }
        $data = $this->prepareFormData();
        $data['title'] = 'Tambah Item Arsip Inaktif';
        return view('item_inaktif/form', $data);
    }

    public function create()
    {
        if (!has_permission('cud_arsip_inaktif')) {
            return redirect()->to('/item-inaktif')->with('error', 'Anda tidak memiliki izin untuk menyimpan data.');
        }

        $postData = $this->request->getPost();

        $dataForValidation = [
            'id_klasifikasi'    => (int)($postData['id_klasifikasi'] ?? 0),
            'id_jenis_naskah'   => (int)($postData['id_jenis_naskah'] ?? 0),
            'id_es3'            => (int)($postData['id_es3'] ?? 0),
            'no_dokumen'        => trim($postData['no_dokumen'] ?? ''),
            'judul_dokumen'     => trim($postData['judul_dokumen'] ?? ''),
            'tgl_dokumen'       => trim($postData['tgl_dokumen'] ?? ''),
            'tahun_cipta'       => (int)($postData['tahun_cipta'] ?? 0),
            'jumlah'            => (int)($postData['jumlah'] ?? 1),
            'tk_perkembangan'   => trim($postData['tk_perkembangan'] ?? ''),
            'lokasi_simpan'     => trim($postData['lokasi_simpan'] ?? ''),
            'media_simpan'      => trim($postData['media_simpan'] ?? ''),
            'no_box'            => trim($postData['no_box'] ?? ''),
            'nama_file'         => trim($postData['nama_file'] ?? ''),
            'nama_folder'       => trim($postData['nama_folder'] ?? ''),
            'nama_link'         => trim($postData['nama_link'] ?? ''),
            'dasar_catat'       => trim($postData['dasar_catat'] ?? ''),

            // <<< INI KUNCI PERBAIKAN: Pastikan nilai defaultnya tidak melanggar 'required' (jika ada) >>>
            'id_user'           => (int)session()->get('user_id'), // User ID tidak pernah null jika sudah login
            'status_arsip'      => 'inaktif', // Default untuk inaktif, cocok dengan ENUM
            'status_pindah'     => 'ba', // Default setelah BA pemindahan, cocok dengan ENUM
            'pinjam'            => 0, // Default 0, cocok dengan TINYINT(1)
        ];


        // --- DEBUGGER: LIHAT ISI $dataForValidation SETELAH KONVERSI ---
        // dd($dataForValidation);
        // Hapus dd() ini setelah analisis selesai.
        // --- AKHIR DEBUGGER ---

        // 2. Ambil aturan validasi LENGKAP dari model
        $rules = $this->itemInaktifModel->getValidationRules();

        // 3. Jalankan validasi dengan data yang sudah dikonversi
        ...
        // --- 4. Jika validasi lolos, lanjutkan penyimpanan ---
        $db = \Config\Database::connect();
        $db->transBegin();

        try {
            // Pastikan id_es2 juga terisi (dari id_es3 yang dipilih)
            if (session()->get('role_access') === 'superadmin') {
                $es3 = $this->unitKerjaEs3Model->find($dataForValidation['id_es3']);
                if ($es3) $dataForValidation['id_es2'] = $es3['id_es2'];
            } else {
                $authData = session()->get('auth_data');
            ...
            if ($this->itemInaktifModel->save($dataForValidation) === false) {
                throw new \Exception('Gagal menyimpan item arsip: ' . implode('<br>', $this->itemInaktifModel->errors()));
            }
            ...
            if ($db->transStatus() === false) {
                ...
                $db->transCommit();
                $this->session->setFlashdata('success', 'Item arsip inaktif berhasil ditambahkan.');
                return redirect()->to('/item-inaktif');
            }
        } catch (\Exception $e) {
            $db->transRollrollbac;
            log_message('error', 'Error create ItemInaktif: ' . $e->getMessage());
    ...
    {
        if (!has_permission('cud_arsip_inaktif')) {
            return redirect()->to('/item-inaktif')->with('error', 'Anda tidak memiliki izin untuk mengubah data.');
        }
        $item = $this->itemInaktifModel->find($id);
        if (!$item) {
            throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound();
        }
        ...
        $data['title'] = 'Edit Item Arsip Inaktif';
    ...
    public function update($id = null)
    {
        ...
        $existingItem = $this->itemInaktifModel->find($id);
        if (!$existingItem) {
            throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound();
        }
        ...
        // --- 2. Ambil aturan validasi LENGKAP dari model ---
        $validationRules = $this->itemInaktifModel->getValidationRules();
        ...
        $dataForValidation = [
            ...
            'lokasi_simpan_new_new'     => $postData['lokasi_simpan_new_new'] ?? null,
        ...
        try {
            if ($this->itemInaktifModel->update($id, $dataToSave) === false) {
                throw new \Exception('Gagal memperbarui item arsip: ' . implode(', ', $this->itemInaktifModel->errors()));
            }
    ...
    {
        if (!has_permission('cud_arsip_inaktif')) {
            return redirect()->to('/item-inaktif')->with('error', 'Anda tidak memiliki izin untuk menghapus data.');
        }
        ...
        if ($item && $item['id_berkas']) {
            ...
            $this->itemInaktifModel->delete($id);
    ...
    public function lepasBerkas()
    {
        if ($this->request->isAJAX()) {
            ...
            try {
                // Lepaskan item
                $this->itemInaktifModel->update($id, ['id_berkas' => null]);

                // Hitung ulang rentang tahun untuk berkas yang ditinggalkan
                $tahunCiptaItems = $this->itemInaktifModel
    ...
    {
        return view('item_inaktif/import');
    }
    ...
    public function prosesImport()
    {
        ...
        if (!$this->validate($validationRules)) {
            ...
            return redirect()->to('/item-inaktif')->withInput()->with('validation', $this->validator);
        }
        ...
        try {
            ...
            foreach ($rows as $rowIndex => $row) {
                ...
                $dataToInsert[] = [
                    ...
                    'lokasi_simpan_new_new'     => $lokasiSimpanNew,
            ...
            if (!empty($dataToInsert)) {
                $inserted = $this->itemInaktifModel->insertBatch($dataToInsert);
            }
            ...
            log_message('error', 'Error prosesImport ItemInaktif: ' . $e->getMessage());
            session()->setFlashdata('show_import_modal', true);
    ...
    public function pinjamItem()
    {
        ...
        $item = $this->itemInaktifModel->find($id_item);
        if (!$item) {
        ...
        try {
            ...
            // Update status di item_inaktif
            $this->itemInaktifModel->update($id_item, ['pinjam' => 1]);
