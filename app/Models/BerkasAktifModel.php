...
class BerkasAktifModel extends BaseModel
{
    ...
    protected $validationRules = [
        ...
        'no_berkas'       => 'is_unique[berkas_aktif.no_berkas,id,{id}]' // Tambahkan ini untuk keunikan
    ];
    protected function generateQrCode(array $data)
    {
        ...
        $qrContentUrl = site_url('berkas-aktif/detail/' . ($id ?? '{id_placeholder}'));
    ...
    protected function updateQrCodeId(array $data)
    {
        ...
        $newQrContentUrl = site_url('berkas-aktif/detail/' . $id);
    ...
    public function getDataTablesList($request)
    {
        ...
        $builder = $this->db->table($this->table . ' as berkas_aktif')
            ->select([
                'berkas_aktif.id',
                ...
                'berkas_aktif.no_box',
                'berkas_aktif.status_tutup',
                'berkas_aktif.pinjam', // <-- PASTIKAN KOLOM INI TERPILIH
                '(SELECT COUNT(*) FROM item_aktif WHERE item_aktif.id_berkas = berkas_aktif.id) as jumlah_item',
            ...
            ->join('unit_kerja_es3 as es3', 'es3.id = berkas_aktif.id_es3', 'left')
            ->join('unit_kerja_es2 as es2', 'es2.id = berkas_aktif.id_es2', 'left')
            ->orderBy('berkas_aktif.id','DESC');
        ...
        if ($session->get('role_access') !== 'superadmin') {
            ...
            if (empty($authData)) {
                ...
                if (!empty($authData['id_es3'])) {
                    $builder->where('berkas_aktif.id_es3', $authData['id_es3']);
                } elseif (!empty($authData['id_es2'])) {
                    $builder->where('berkas_aktif.id_es2', $authData['id_es2']);
                } elseif (!empty($authData['id_es1'])) {
                    $db = \Config\Database::connect();
                    $subQueryEs2 = $db->table('unit_kerja_es2')->select('id')->where('id_es1', $authData['id_es1']);
                    $builder->whereIn('berkas_aktif.id_es2', $subQueryEs2);
                } else {
                    $builder->where('1=0');
                }
            }
        ...
        $column_search = [
            ...
            'berkas_aktif.no_berkas',
            'berkas_aktif.nama_berkas',
            'berkas_aktif.status_tutup' // Tambahkan status ke pencarian
        ];
